...
from app.admin.decorators import admin_required, valid_admin_required, check_delete, check_edit
from app.admin.forms import  StudyForm, CardSetForm, UserGroupForm
from app.models import Card, CardSet, DataValueLabel, HeatMap, Participant, Study, User, UserGroup
...
def index():
    ...
    return render_template('admin/index.html', studies=studies, card_sets=card_sets, user_groups=user_groups)
...
def study(id, study):
    ...
    form.user_group.query = UserGroup.query.filter(UserGroup.creator == current_user).filter( or_(UserGroup.study==study, UserGroup.study==None))
        ...
        for card in form.cards.data:
            ...
            if file is not None:
                ...
                if not os.path.exists(os.path.dirname(file_path)):
                    os.makedirs(os.path.dirname(file_path), exist_ok=True)
                file.save(file_path)
                card_list.append(Card(name=card.get("card_name"), description= card.get('desc'), image=file_name, creator=current_user))
            else:
                
                card_list.append(Card(name=card.get("card_name"), description= card.get('desc'), creator=current_user))
