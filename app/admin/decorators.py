...
def check_delete(function):
    ...
    def wrapper3(*args, **kwargs):
        if 'user_group' in kwargs:
            ...
            if user_group.study: 
                string = 'You cannot delete this User Group as it is currently associated with {study} Study, remove this association before deleting this User Group.'.format(study=user_group.study)
                flash(string)
        ...
        if 'card_set' in kwargs:
            ...
            if card_set.studies_x:
                ...
                string = 'You cannot delete this Card Set as it is currently associated with {studies} Studies, remove these associations before deleting this Card Set.'.format(studies=studies_str)
                flash(string)
                return redirect(url_for('admin.index'))
        ...
        if 'user_group' in kwargs:
            ...
            if user_group.study: 
                if not check_study_date([user_group.study]):
                    flash('You cannot edit this User Group as the Study associated with it is currently in progress.')
                    return redirect(url_for('admin.index'))
                else:
                    return function(*args, **kwargs)
        if 'card_set' in kwargs:
            ...
            if card_set.studies_x:
                if not check_study_date(card_set.studies_x):
                    flash('You cannot edit this Card Set as there are Studies associated with it which are currently in progress.')
                    return redirect(url_for('admin.index'))
                else:
                    return function(*args, **kwargs)
