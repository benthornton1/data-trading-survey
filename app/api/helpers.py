...
def create_participant_json(user):
    json = {'id':user.id, 'username':user.username, 'type':user.type,
             'gender':user.gender, 'age_group':user.age_group, 'country_of_birth':user.country_of_birth,
             'education_level':user.education_level, 'occupation':user.occupation, 'income':user.income,
...
def create_card_set_json(card_set):
    json = {
        'id':card_set.id, 'name':card_set.name, 'measure':card_set.measure,
...
def create_data_value_label_json(data_value_label):
    json = {
            'id':data_value_label.id,'label':data_value_label.label,
...
def create_study_json(study):
    json = {
            ...
            'image':study.image, 'card_set_x':create_card_set_json(study.card_set_x),
            'card_set_y':create_card_set_json(study.card_set_y), 
            'data_value_labels':[create_data_value_label_json(data_value_label) for data_value_label in study.data_value_labels],
...
def create_heat_map_json(heat_map):
    if heat_map.data_value_label:
        json = {'id':heat_map.id, 'card_x':create_card_json(heat_map.card_x),
            'card_y':create_card_json(heat_map.card_y), 'study_id':heat_map.study_id,
            'values':heat_map.values,'data_value_label_id':heat_map.data_value_label.id,
            'is_count':heat_map.is_count
        }
    else:
        json = {'id':heat_map.id, 'card_x':create_card_json(heat_map.card_x),
            'card_y':create_card_json(heat_map.card_y), 'study_id':heat_map.study_id,
            'values':heat_map.values,'data_value_label_id':None,
            'is_count':heat_map.is_count
        }
...
def create_user_group_json(user_group):
    json = {}
    if user_group.study:
        json = {'id':user_group.id, 'name':user_group.name,
