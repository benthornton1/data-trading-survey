...
class Participant(User):
    ...
    user_group = db.relationship('UserGroup', foreign_keys=[user_group_id], backref=backref("users", cascade="all,delete"))
...
class UserGroup(db.Model):
    ...
    creator = db.relationship('Admin', foreign_keys=[creator_id], backref='user_groups')    
    study = db.relationship('Study', uselist=False, backref=backref("user_group", cascade="all,delete"))
...
class Study(db.Model):
    ...
    card_set_x = db.relationship('CardSet', foreign_keys=card_set_x_id, backref='studies_x') 
    card_set_y = db.relationship('CardSet', foreign_keys=card_set_y_id, backref='studies_y')
    data_value_labels = db.relationship('DataValueLabel', backref='study', cascade='all, delete')
    heat_maps = db.relationship('HeatMap', backref='study', cascade='all, delete')
    responses = db.relationship('Response', backref='study', cascade='all, delete')
...
class Response(db.Model):
    ...
    creator = db.relationship('Admin', foreign_keys=creator_id, backref='responses')
    participant = db.relationship('Participant', foreign_keys=participant_id, backref='response')
...
class HeatMap(db.Model):
    ...
    data_value_label_id = db.Column(db.Integer, db.ForeignKey('data_value_label.id'))
    creator_id = db.Column('Admin', db.ForeignKey('user.id'))
    study_id = db.Column(db.Integer, db.ForeignKey('study.id'))
