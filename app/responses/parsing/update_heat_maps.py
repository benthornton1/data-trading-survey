        ...
        for col_row, values in data_values.items():
            for idx, value in enumerate(values):
                if value.value != "" and value.value != None:
                    ...
                    try:
                        if idx == 0:
                            new_val = (value.value - min_data_value_0) / (
                                max_data_value_0 - min_data_value_0
                            )
                        if idx == 1:
                            new_val = (value.value - min_data_value_1) / (
                                max_data_value_1 - min_data_value_1
                            )
                    except:
                        new_val = 1
...
def update_heat_maps(response):
    ...
    heat_maps = HeatMap.query.filter(
        HeatMap.study == response.study, HeatMap.is_count == False
    ).all()
    ...
    heat_maps_count = (
        ...
        .filter(HeatMap.study_id == response.study_id)
        .filter(HeatMap.is_count == True)
        .all()
    )
    for col, cards_x in response_cards_x.items():
        for row, cards_y in response_cards_y.items():
