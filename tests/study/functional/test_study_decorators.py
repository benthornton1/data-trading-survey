...
def test_valid_participant_required_valid(client, init_database):
    ...
    with client.application.test_request_context():
        ...
        participant = create_participant(client, username='participant', password='password', completed_form=True)
...
def test_check_complete_study(client, init_database):
    ...
    with client.application.test_request_context():
        admin = create_admin(client)
        participant = create_participant(client,username='p', password='p', completed_form=True)
        user_group = create_user_group(client, creator=admin, participants=[participant])
        study = create_study(client, creator=admin, user_group=user_group)
        ...
        response = client.get(url_for('study.index', id=study.id), follow_redirects=False)
        assert urlparse(response.location).path == url_for('study.complete')
        response = client.get(url_for('study.study', id=study.id), follow_redirects=False)
        assert urlparse(response.location).path == url_for('study.complete')
        response = client.get(url_for('study.user_info', id=study.id), follow_redirects=False)
        assert urlparse(response.location).path == url_for('study.complete')
        response = client.get(url_for('study.change_info', id=study.id), follow_redirects=False)
        assert urlparse(response.location).path == url_for('study.complete')
...
def test_participant_required(client, init_database):
    ...
    with client.application.test_request_context():
        ...
        response = client.get(url_for('study.change_info', id=study.id), follow_redirects=False)
        assert urlparse(response.location).path == url_for('auth.login')
        response = client.get(url_for('study.complete', id=study.id), follow_redirects=False)
        assert urlparse(response.location).path == url_for('auth.login')
        client.get(url_for('auth.logout'))
...
def test_complete_info_form_required(client, init_database):
    ...
    """ 
    with client.application.test_request_context():
        participant = create_participant(client, username='p', password='p', completed_form=False)
