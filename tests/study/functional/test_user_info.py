...
def test_get_user_info(client, init_database):
    ...
    with client.application.test_request_context():
        ...
        participant = create_participant(client, username='participant', password='password')
        ...
        response = client.get(url_for('study.user_info'), follow_redirects=True)
        form = UserInfoForm()
...
def test_post_user_info(client, init_database):
    ...
    with client.application.test_request_context():
        ...
        repsonse = client.post(url_for('study.user_info'), data=form.data, follow_redirects=True)
        assert response.status_code == 200
