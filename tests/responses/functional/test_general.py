...
def test_get_general_study(client, init_database):
    ...
    with client.application.test_request_context():
        ...
        study_response = create_response(client, participant=participant, creator=admin, study=study)
        create_heat_maps(client, creator=admin, study=study, responses=[study_response])
        
        login(client, username='admin', password='password')

        response = client.get(url_for('responses.general', id=study.id), follow_redirects=True)
        
        assert response.status_code == 200
        assert bytes(str(len(study.responses))+ ' Responses', 'utf-8') in response.data
        assert bytes(str(len(study.heat_maps))+ ' Heat Maps', 'utf-8') in response.data
...
def test_get_general_no_study(client, init_database):
    ...
    with client.application.test_request_context():
        ...
        response = client.get(url_for('responses.general', id=1), follow_redirects=True)
