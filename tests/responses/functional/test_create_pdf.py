...
from tests.helpers import create_admin, login, create_user_group, create_participant, create_response, create_study
...
def test_get_create_pdf(client, init_database):
    ...
    with client.application.test_request_context():
        ...
        response = client.get(url_for('responses.create_pdf', id=study.id), follow_redirects=True)
        
        assert response.status_code == 200
        assert b'Create PDF' in response.data
        assert b'<p>Here you can generate a PDF and select what responses you want to include in the pdf</p>' in response.data
