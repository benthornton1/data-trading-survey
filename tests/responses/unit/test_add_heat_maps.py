...
from tests.helpers import create_admin, create_data_value_label, create_participant, create_response, create_study, create_user_group
...
def test_CreateOneHeatMap(client, init_database):
    ...
    study = create_study(client, creator=admin, user_group=user_group, data_value_labels=[])
    ...
    c = CreateOneHeatMap()
    c.add(is_count=False, study=study, card_x_id=study.card_set_x.cards[0].id, card_y_id=study.card_set_y.cards[1].id)
    plot = c.plots
    ...
    c.add(is_count=True, study=study, card_x_id=study.card_set_x.cards[0].id, card_y_id=study.card_set_y.cards[1].id)
    plot = c.plots
    ...
    user_group = create_user_group(client, participants=[participant_2], creator=admin)
    study_2 = create_study(client, creator=admin, user_group=user_group)
    response = create_response(client, study=study, creator=admin, participant=participant_2)
    
    find_combinations(study_2)
    update_heat_maps(response)
    c = CreateOneHeatMap()
    c.add(is_count=False, study=study_2, card_x_id=study_2.card_set_x.cards[0].id, card_y_id=study_2.card_set_y.cards[1].id, label=study_2.data_value_labels[0])
    plot = c.plots
    ...
    c.add(is_count=True, study=study_2, card_x_id=study_2.card_set_x.cards[0].id, card_y_id=study_2.card_set_y.cards[1].id)
    plot = c.plots
...
def test_CreateAllHeatMaps(client, init_database):
    ...
    assert len(plots) == len(study.card_set_x.cards)*len(study.card_set_y.cards)*len(study.data_value_labels)
    ...
    assert len(plots) == len(study_2.card_set_x.cards)*len(study_2.card_set_y.cards)*len(study_2.data_value_labels)
