...
from app import db
from app.models import HeatMap, CardSet, Response, Card, Admin, Participant, UserGroup, DataValueLabel, Study
...
def login(client, username='participant', password='password'):
    response = client.post(url_for('auth.login'), 
                            json=dict(username=username, password=password), follow_redirects=True)
    assert response.status_code == 200
...
def create_participant(client, username='participant', password='password', email='test@test.com', gender=None, age_group=None, country_of_birth=None, latest_country=None,
                       education_level=None, occupation=None, completed_form=False, completed_study=False):
    with client.application.test_request_context():
        participant = Participant(username=username, email=email, gender=gender, age_group=age_group,
                                country_of_birth=country_of_birth, latest_country=latest_country, education_level=education_level,
                                occupation=occupation, completed_form=completed_form, completed_study=completed_study)
...
def create_study(client, name="Test Study", description="", image="", data_values=0, number_of_columns=4, number_of_rows=4, start_date=date.today(), end_date=date.today()+timedelta(days=3),
                mail_sent=False, creator=None, user_group=None, card_set_x=None, card_set_y=None, data_value_labels=None):
        with client.application.test_request_context():
            if not creator:
            ...
            if not data_value_labels:
                data_value_labels = [create_data_value_label(client, creator=creator)]
            study = Study(name=name, description=description, image=image, data_values=data_values, number_of_columns=number_of_columns, number_of_rows=number_of_rows,
                                start_date=start_date, end_date=end_date, mail_sent=mail_sent, creator=creator, user_group=user_group, card_set_x=card_set_x, card_set_y=card_set_y,
...
def create_user_group(client, name='Test User Group', creator=None, study=None, participants=None):
    with client.application.test_request_context():
        if not creator:
            creator=create_admin(client)
        if not participants:
            participants=[create_participant(client)] 
        user_group = UserGroup(name=name, creator=creator, study=study, users=participants)
        db.session.add(user_group)
        db.session.commit()
        
        return user_group
    
def create_data_value_label(client, label='Test Data Value Label', creator=None):
    with client.application.test_request_context():
        if not creator:
            creator = create_admin(client)
...
def create_card_set(client, creator=None, cards=None, name="Test Card Set", measure="Sensitvity"):
    with client.application.test_request_context():
        if not creator:
            creator = create_admin(client)
        if not cards:
            cards = create_cards(client, creator=creator)
        card_set = CardSet(name=name, measure=measure, creator=creator, cards=cards)
        db.session.add(card_set)
        db.session.commit()
        
        return card_set
    
def create_cards(client, creator=None, name="Test Card", description="", image=""):
    with client.application.test_request_context():
        if not creator:
            creator = create_admin(client)
        card1 = Card(name="Entertainment", description="Music/ Video/ TV", image='yay.jpg', creator=creator)
        ...
        card5 = Card( name="Household", description="Energy/ water consumption. Fridge/ Food Cabinet", image='yay.jpg', creator=creator)
        card6 = Card( name="Service Contracts", description="Gym/ Telephone/ Mobile/ Internet/ TV", image='yay.jpg', creator=creator)
        ...
        card11 = Card( name="Food and Beverage", description="", image='yay.jpg', creator=creator)
        card12 = Card( name="Location", description="", image='yay.jpg', creator=creator)
        ...
        card17 = Card( name="Education Institute", description="", image='hi.jpg', creator=creator)
        ...
        card21 = Card( name="Religious Organisation", description="", image='hi.jpg', creator=creator)
...
def create_card(client, creator=None, name="Test Card", description="Test description", image="yay.jpg"):
   with client.application.test_request_context():
        if not creator:
            creator = create_admin(client)
        card = Card(name=name, description=description, image=image, creator=creator) 
        db.session.add(card)
        db.session.commit()
        return card
def create_response(client, cards_x=None, cards_y=None, data_values=None, participant=None, creator=None, study=None):
    with client.application.test_request_context():
        if not participant:
        ...
        if not creator:
            creator = create_admin(client)
        if not cards_x:
            cards_x = {'col_0': [{'id': 10, 'name': 'Financial', 'image': 'yay.jpg', 'description': 'Bank Statements/ Contracts/ Life Insurance/ Vehicle/ Property'}, {'id': 8, 'name': 'Fashion', 'image': 'yay.jpg', 'description': 'Wardrobe/ Outfits'}, {'id': 11, 'name': 'Food and Beverage', 'image': 'yay.jpg', 'description': ''}, {'id': 7, 'name': 'GP Health Records', 'image': 'yay.jpg', 'description': ''}], 'col_1': [{'id': 9, 'name': 'Communications', 'image': 'yay.jpg', 'description': 'Calls/ Messages/ Chats'}, {'id': 6, 'name': 'Service Contracts', 'image': 'yay.jpg', 'description': 'Gym/ Telephone/ Mobile/ Internet/ TV'}, {'id': 12, 'name': 'Location', 'image': 'yay.jpg', 'description': ''}], 'col_2': [{'id': 4, 'name': 'Demographics Data', 'image': 'yay.jpg', 'description': 'Age, Gender, Political Opinions, Religion, Height, Weight etc'}, {'id': 3, 'name': 'Activity and Sleep', 'image': 'yay.jpg', 'description': ''}, {'id': 5, 'name': 'Household', 'image': 'yay.jpg', 'description': 'Energy/ water consumption. Fridge/ Food Cabinet'}], 'col_3': [{'id': 1, 'name': 'Entertainment', 'image': 'yay.jpg', 'description': 'Music/ Video/ TV'}, {'id': 2, 'name': 'Sensitive Health Report', 'image': 'yay.jpg', 'description': 'Blood Report/ Fertility Test/ DNA Test'}]}
        if not cards_y:
            cards_y = {'row_3': [{'id': 18, 'name': 'Government Regulated', 'image': 'hi.jpg', 'description': 'Royal Mail, Network Rail'}, {'id': 13, 'name': 'Mortgage', 'image': 'hi.jpg', 'description': ''}, {'id': 25, 'name': 'Banks', 'image': 'hi.jpg', 'description': ''}, {'id': 22, 'name': 'Supermarket', 'image': 'hi.jpg', 'description': 'Tesco, Sainsburys, Lidl, etc'}, {'id': 15, 'name': 'Products and Services', 'image': 'hi.jpg', 'description': ''}], 'row_2': [{'id': 16, 'name': 'Insurance', 'image': 'hi.jpg', 'description': 'Car insurers, Home insurers'}, {'id': 14, 'name': 'Research Institute', 'image': 'hi.jpg', 'description': ''}, {'id': 27, 'name': 'Electricity', 'image': 'hi.jpg', 'description': 'Electricity Companies'}, {'id': 26, 'name': 'Government', 'image': 'hi.jpg', 'description': ''}, {'id': 24, 'name': 'Council', 'image': 'hi.jpg', 'description': 'i.e. Cardiff City Council'}], 'row_1': [{'id': 20, 'name': 'Health Service', 'image': 'hi.jpg', 'description': 'NHS'}, {'id': 21, 'name': 'Religious Organisation', 'image': 'hi.jpg', 'description': ''}, {'id': 17, 'name': 'Education Institute', 'image': 'hi.jpg', 'description': ''}], 'row_0': [{'id': 19, 'name': 'Social Media', 'image': 'hi.jpg', 'description': 'Facebook, Snapchat, Instagram, Twitter, etc'}, {'id': 23, 'name': 'Authorities', 'image': 'hi.jpg', 'description': 'Police'}]}
        if not data_values:
            data_values = {'col_0_row_3': [{'id': 1, 'label': 'Test Data Value Label', 'value': 43}, {'id': 2, 'label': 'Data Consumer gets the benefit (e.g. Supply Chain gets the benefit)', 'value': 63}], 'col_1_row_3': [{'id': 1, 'label': 'You get the benefit (e.g. Personalised Service)', 'value': 82}, {'id': 2, 'label': 'Data Consumer gets the benefit (e.g. Supply Chain gets the benefit)', 'value': 72}], 'col_2_row_3': [{'id': 1, 'label': 'You get the benefit (e.g. Personalised Service)', 'value': 82}, {'id': 2, 'label': 'Data Consumer gets the benefit (e.g. Supply Chain gets the benefit)', 'value': 99}], 'col_3_row_3': [{'id': 1, 'label': 'You get the benefit (e.g. Personalised Service)', 'value': 71}, {'id': 2, 'label': 'Data Consumer gets the benefit (e.g. Supply Chain gets the benefit)', 'value': 72}], 'col_0_row_2': [{'id': 1, 'label': 'You get the benefit (e.g. Personalised Service)', 'value': 42}, {'id': 2, 'label': 'Data Consumer gets the benefit (e.g. Supply Chain gets the benefit)', 'value': 52}], 'col_1_row_2': [{'id': 1, 'label': 'You get the benefit (e.g. Personalised Service)', 'value': 67}, {'id': 2, 'label': 'Data Consumer gets the benefit (e.g. Supply Chain gets the benefit)', 'value': 89}], 'col_2_row_2': [{'id': 1, 'label': 'You get the benefit (e.g. Personalised Service)', 'value': 52}, {'id': 2, 'label': 'Data Consumer gets the benefit (e.g. Supply Chain gets the benefit)', 'value': 62}], 'col_3_row_2': [{'id': 1, 'label': 'You get the benefit (e.g. Personalised Service)', 'value': 66}, {'id': 2, 'label': 'Data Consumer gets the benefit (e.g. Supply Chain gets the benefit)', 'value': 72}], 'col_0_row_1': [{'id': 1, 'label': 'You get the benefit (e.g. Personalised Service)', 'value': 82}, {'id': 2, 'label': 'Data Consumer gets the benefit (e.g. Supply Chain gets the benefit)', 'value': 65}], 'col_1_row_1': [{'id': 1, 'label': 'You get the benefit (e.g. Personalised Service)', 'value': 82}, {'id': 2, 'label': 'Data Consumer gets the benefit (e.g. Supply Chain gets the benefit)', 'value': 32}], 'col_2_row_1': [{'id': 1, 'label': 'You get the benefit (e.g. Personalised Service)', 'value': 42}, {'id': 2, 'label': 'Data Consumer gets the benefit (e.g. Supply Chain gets the benefit)', 'value': 82}], 'col_3_row_1': [{'id': 1, 'label': 'You get the benefit (e.g. Personalised Service)', 'value': 92}, {'id': 2, 'label': 'Data Consumer gets the benefit (e.g. Supply Chain gets the benefit)', 'value': 73}], 'col_0_row_0': [{'id': 1, 'label': 'You get the benefit (e.g. Personalised Service)', 'value': 43}, {'id': 2, 'label': 'Data Consumer gets the benefit (e.g. Supply Chain gets the benefit)', 'value': 77}], 'col_1_row_0': [{'id': 1, 'label': 'You get the benefit (e.g. Personalised Service)', 'value': 28}, {'id': 2, 'label': 'Data Consumer gets the benefit (e.g. Supply Chain gets the benefit)', 'value': 32}], 'col_2_row_0': [{'id': 1, 'label': 'You get the benefit (e.g. Personalised Service)', 'value': 12}, {'id': 2, 'label': 'Data Consumer gets the benefit (e.g. Supply Chain gets the benefit)', 'value': 44}], 'col_3_row_0': [{'id': 1, 'label': 'You get the benefit (e.g. Personalised Service)', 'value': 66}, {'id': 2, 'label': 'Data Consumer gets the benefit (e.g. Supply Chain gets the benefit)', 'value': 34}]}
        response = Response(cards_x=cards_x, cards_y=cards_y, data_values=data_values, creator=creator, participant=participant, study=study)
