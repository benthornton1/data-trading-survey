...
from tests.helpers import create_admin, create_card_set, create_study, create_user_group, login
...
def test_create_valid_study(client, init_database):
    ...
    with client.application.test_request_context():
        ...
        response = client.get(url_for('admin.new_study'), follow_redirects=True)
        ...
        assert bytes(form.number_of_columns.label.text, 'utf-8') in response.data
        ...
        response = client.post(url_for('admin.study', id=1), data=form.data, follow_redirects=True)
        ...
        assert len(study_db.heat_maps) == (len(study_db.card_set_x.cards)*len(study_db.card_set_y.cards)*len(study_db.data_value_labels))+(len(study_db.card_set_x.cards)*len(study_db.card_set_y.cards))
...
def test_create_invalid_study(client, init_database):
    ...
    with client.application.test_request_context():
        ...
        assert b'The end date cannot be before the start date.' in response.data
        assert b'The end date cannot be in the past.' in response.data
        assert b'Card Set for x-axis cannot have the same value as Card Set for y-axis.' in response.data
...
def test_delete_current_study(client, init_database):
    ...
    with client.application.test_request_context():
        ...
        response = client.get(url_for('admin.delete_study', id=study.id), follow_redirects=True)
        
        assert response.status_code == 200
        assert b"You cannot delete this Study as it is currently in progress." in response.data
...
def test_delete_future_study(client, init_database):
    ...
    with client.application.test_request_context():
        ...
        study = create_study(client, name="Test Study", creator=admin, start_date=date.today()+timedelta(days=1))
...
def test_edit_current_study(client, init_database):
    ...
    with client.application.test_request_context():
        ...
        response = client.get(url_for('admin.study', id=study.id), follow_redirects=True)
        
        assert response.status_code == 200
        assert b"You cannot edit this Study as it is currently in progress." in response.data
...
def test_edit_future_study(client, init_database):
    ...
    with client.application.test_request_context():
        ...
        study = create_study(client, start_date=date.today()+timedelta(days=3), creator=admin)
        response = client.get(url_for('admin.study', id=1), follow_redirects=True)
        ...
        response = client.post(url_for('admin.study', id=study.id), data=form.data, follow_redirects=True)
        ...
        assert len(updated_study.heat_maps) == (len(study.card_set_x.cards)*len(study.card_set_y.cards)*len(study.data_value_labels))+(len(study.card_set_x.cards)*len(study.card_set_y.cards))
        assert len(updated_study.data_value_labels) == len(study.data_value_labels)